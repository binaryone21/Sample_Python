...
# 간단한 텍스트 응답
@app.route('/simpleText', methods=['GET', 'POST'])
def text():
    res = {
        "version": "2.0",
        "template": {
...
# 간단한 이미지 응답
@app.route('/SimpleImage', methods=['GET', 'POST'])
def image():
    res = {
        "version": "2.0",
        "template": {
...
@app.route('/plugImage', methods=['GET', 'POST'])
def imgPlug():
...
@app.route('/plugProfile', methods=['GET', 'POST'])
def profilePlug():
    param = request.get_json()
...
# 응용 현재위치 이용해서 ...
@app.route('/findGPS', methods=['GET', 'POST'])
def findGPS():
    # param = request.get_json()
    # seoulLocal = param['action']['detailParams']['centerName']['origin']
